...
import { rotateArray, pitchColor, chromaColorMix, globalScale, notes } from 'use-chromatone'
...
.flex.flex-col.justify-center.rounded-3xl.py-2(
  ...
  .flex.gap-1.p-2.text-lg(v-if="title")
    ...
    .flex-1
    .p-0.capitalize.active-op-100.transition(v-for="(ins, i) in instruments" :key="ins" @click="instrument = i" :class="{ [i == instrument ? 'op-100' : 'op-40']: true }") {{ i }}
